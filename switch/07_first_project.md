...
한 문장: **90일은 "작은 것을 끝까지 배포한 경험"을 만드는 기간이지, 지식을 모으는 기간이 아닙니다.**

## 7.2 90일 계획 (3막)

### 1막 (1~3주): 손에 익히기 — 📕책과 함께

- 📕[PyTorch 도서](../book/README.md) 01~05장. **읽지 말고 치세요.** 코드 다 실행.
- 이 책 01·03·04·05장의 코드를 전부 직접 실행하고 값을 확인.
- 결과물: 📕 03장 실험과 같은 **학습률 발산 곡선을 스스로 만든 화면**. 이게 첫 "이해했다"의 증거.
...
여기가 진짜입니다. **모델은 이 주기의 한 칸일 뿐**입니다.

주제: **실제 데이터 하나를 직접 모아 분류하고 배포하기.**
예시 후보 (이미지·텍스트 중 익숙한 쪽으로 하나):
...
2막 필수 과제 (순서대로):
1. **데이터 수집 + 라벨링 스키마 설계** (03장). 라벨 정의 문서 1장.
2. **상수 베이스라인 만들기** (05.3). 다수 클래스 예측의 정확도. **여기서 시작점 0을 정의.**
3. **피처를 직접 만들어 베이스라인 깨기** (01장 A0!). 모델 없이 피처만으로 어디까지 되는지.
4. 피처로 안 되는 부분만 모델로 (📕04·06). train/val/test 3분할 + 층화 (03.3).
5. **지표는 5.4 체크리스트 6개로 보고.**
6. `/predict`로 배포 (06장). 응답에 `confidence`·`model_version` 포함.
...
이 저장소 로드맵 Phase 4·5로 넘어가기 위한 다리입니다.

- OpenCV(`03_ai_vision/opencv/`)로 **실제 프레임 → tensor** 변환 파이프라인 작성.
  `cv2.imread`는 `(H,W,C)` BGR, PyTorch는 `(C,H,W)` RGB → `permute`/변환 지점 확보 (📕06장 6.1).
- 시뮬레이션/웹캠 입력에서 추론 → 다음 장으로 값 전달. **제어 루프 안의 지연 예산**을 직접 재보기.
  06장 실측(2ms)과 네트워크 지터의 차이를体感하는 순간 로드맵이 현실이 됩니다.
- 안전 계층: 추론이 틀렸을 때의 fallback을 **코드로** 구현 (비상정지·속도제한·임계값 미달 시 사람).
  5.1의 "17건 통과"가 로봇에서는 물리적 사건입니다.

## 7.3 포트폴리오로成立的인 형태

"모델 만들었습니다"는 안 읽힙니다. 읽히는 건 **결론과 근거**입니다.
...
- **하지 않은 것을 쓰는** 게 핵심입니다. 선택의 근거 = 엔지니어링 능력의 증거.
- 재현이 안 되면 포트폴리오가 아닙니다. 02장 표의 `manual_seed`·버전 고정이 여기서 값이 나옵니다.
- Physical AI 방향이면 **영상/GIF 1개**가 문서 10페이지보다 강합니다. 시뮬레이션 캡처라도 반드시 넣으세요.
...
90일간 **커밋이 40개** 이상이면 대부분 성공합니다. 5개면 읽기만 한 것입니다.
...
- `confidence` 낮은 샘플을 사람이 고쳐 라벨을 갱신하는 루프가 **단 one의 실력 엔진**입니다.
