...
여기까지 오시면 "아직 서비스로 나가긴 멀었어요"라고들 하세요. 아니에요. 추론은 그냥 HTTP API입니다.
당신이 이미 하는 일이고, 00장이 약속한 "이곳에서 당신이 이기는 지점"이 실제로 나오는 장이기도
하죠.

`fastapi`·`uvicorn`이 이 머신에 없어서 살짝 김이 새실 수 있어요. 근데 아래처럼 표준 라이브러리
`http.server`만 써도 결론은 똑같이 납니다. 프레임워크는 껍데기이고, 병목은 그 바깥에 있거든요.

## 6.1 배포 산출물은 2개다

04장에서 본 대로, 배포 대상은 코드가 하나만이 아니에요.
...
이 둘의 짝이 맞아야 합니다. shape은 딱 맞는데 뜻은 다른 값을 얹으면, 에러는 하나도 안 나고 틀린 답만
나와요.
...
- `state_dict`만 저장하면 이식성이 가장 좋고, 역직렬화 코드가 실행될 위험도 없어요.
- `weights_only=True`는 로드 시 임의 코드 실행을 막습니다. **서드파티 파일을 받을 때 필수**예요.
- `eval()`은 04장 실측대로 이게 빠지면 같은 입력에 다른 답이 나옵니다.

## 6.2 서빙은 상태가 있는 프로세스다

가장 흔한 배포 사고 3개를 모아봤어요. 전부 당신이 이미 아는 패턴이니까 부담 없이 보시면 됩니다.
...
`Handler.model`처럼 전역에 한 번 올려두는 게 정답이에요. 아래 실측이 그 근거입니다.

## 6.3 실측 — 순수 추론 vs HTTP 왕복 vs 나쁜 클라이언트

`/predict` 엔드포인트를 만들어서 01장의 원형 분류 모델을 올려놓고, 세 가지로 나눠서 재봤습니다.

```python
import httpx, time, torch

# (1) HTTP를 전부 제외하고 모델.forward 만 1000회
...
```text
pure inference   = 0.022 ms/req
HTTP warm client = 1.75 ms/req
HTTP new-client  = 328.00 ms/req  <- 클라이언트 생성 오버헤드
```

> 세 값은 루프백·동일 머신·같은 코드에서의 1회 측정입니다. **절대치는 재실행마다 흔들립니다**
> (순수 추론은 ±0.01ms, 왕복은 ±0.5ms 수준). 이 장의 결론은 절대치가 아니라 아래 **배수**입니다.

결론은 크게 한 줄로 쓸게요. 병목은 모델이 아니라 클라이언트 사용법입니다.

- 모델 계산만: 0.022 ms
- HTTP 왕복(연결 재사용): 1.75 ms → 모델의 80배, 그래도 2ms 미만이에요.
- 클라이언트를 매번 생성: 328.00 ms → 모델의 약 **15,000배**

"추론이 느려요"라는 이야기의 상당수가 위 목록의 (3)번이에요. 이걸 잡는 데 ML 지식은 0이 필요하고
HTTP 커넥션 풀 상식만 필요합니다. 00장 0.3표가 여기서 payoff되는 거죠.

<!-- diagrams:inserted -->

세 값을 한 축에 올려보면 결론을 말로 설명할 필요도 없습니다. 값은 모두 이 머신 실측이에요.

```mermaid
xychart-beta
    title "request당 소요 시간 ms (실측)"
    x-axis ["순수 추론", "HTTP 재사용", "클라 매번 생성"]
    y-axis "ms" 0 --> 340
    bar [0.022, 1.75, 328.0]
```

비율만 보면 됩니다. 순수 추론 0.022 ms를 기준으로 재사용은 약 80배, 매번 생성하면 약 15,000배예요. 그리고 요청 한 번이 실제로 지나는 길은 이겁니다.

```mermaid
flowchart LR
    C["클라이언트"] -->|"POST /predict"| H["HTTP 서버<br/>모델은 기동 시 1회 load"]
    H --> L["load_state_dict + eval()"]
    L --> F["no_grad 안에서 forward"]
    F --> P["softmax → label + confidence"]
    P -->|"JSON 응답"| C
    P -.->|"confidence 낮음"| HUM["사람 검토 큐로 라우팅"]
    HUM -.->|"수정된 라벨"| TRAIN["다음 학습 데이터"]
```
...
`label`만 던지지 말고 `confidence`를 같이 실어 주세요. 확신이 낮은 요청을 사람이 처리하도록
라우팅하는 최소 장치가 그 하나입니다. distribution drift를 감지하는 유일한 무료 신호이기도 하고요
(→ 05장 5.4 체크리스트의 "모니터링").
...
마지막 줄이 Physical AI에서 결정적입니다. 로봇 제어 루프가 50Hz(20ms 주기)라면 서버 왕복 2ms도
쫄깃하지만, 네트워크 지터는 치명적으로 바뀌거든요. 그래서 이 저장소 상위 로드맵이
...
`fastapi` 없이도 결론이 같다는 걸 보이려는 의도예요.
...
**이 코드에서 ML인 부분은 한 줄**입니다(`self.model(...)`). 나머지는 전부 백엔드예요. 이 장의 결론이
그 한 줄이라는 게 좀 민망하지만, 사실입니다.

## 정리

1. 배포 산출물은 **아키텍처(코드) + 가중치·전처리 상수(파일)** 두 개이고 짝 맞춰 올려야 한다.
2. 서빙 프로세스는 상태(가중치)를 가진다. 기동 시 1회 load, 워커 간 동일 버전, 갱신 금지.
3. 실측: 추론 0.022ms, 왕복 1.75ms, **클라이언트 재생성 328.00ms.** 느리다고 느끼는 건 대부분 셋째.
...
3. 6.4의 응답 캐시를 붙이고 적중률을 측정해 보세요. 이 문제에선 몇 %가 히트하나요?
