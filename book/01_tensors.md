...
Tensor는 **같은 타입의 숫자들을 격자 형태로 담은 배열**에, PyTorch가 두 가지 능력을 더한 것입니다.

1. 연산 오버로드 (`+`, `*`, `@`) 와 브로드캐스팅.
2. 기울자 추적 (`requires_grad`) — 02장.

`numpy`를 알아도 tensor를 새로 배워야 하는 이유가 2번입니다. 1번은 거의 같습니다.
...
- `shape`(모양): 각 축의 길이. 딥러닝 디버깅의 90%는 shape 불일치입니다.
- `dtype`: 기본은 `float32`. 정수 클래스 레이블은 `long`(=int64)을 씁니다(06장).
- `device`: `cpu` 또는 `cuda`. 연산에 쓰는 두 tensor의 device와 dtype은 ** 같아야 **합니다.
...
`.item()`은 **원소 하나짜리 tensor를 파이썬 스칼로로** 빼냅니다. 학습 중 loss를 출력할 때 필수.

## 1.2 원소별 연산 vs 행렬 곱

가장 자주 헷갈리는 지점. `*`와 `@`는 완전히 다릅니다.
...
- `a @ a`: 선형대수의 행렬곱. `(1·1+2·3, 1·2+2·4 / 3·1+4·3, 3·2+4·4)`.
- `a * a`: 위치별 제곱. 03장의 손실 `(pred - y)**2` 여기서 나옵니다.
- `a.mean()`: 스칼라 손실을 만들 때 쓰는 평균.

### shape 규칙 (선형대수 복습 겸)

`(n, m) @ (m, k) → (n, k)`. 내측 `m`끼리 맞아야 하고, 결과의 외측만 남습니다.
이 규칙을 종이에 적어두면 신경망 코드의 shape 추적이 쉬워집니다.

## 1.3 Broadcasting — 서로 다른 모양도 더한다

두 tensor의 shape이 완전히 일치하지 않아도, 크기가 `1`인 축은 자동으로 늘려 연산합니다.
...
`(2,2)` tensor에 `(2,1)`을 더하면, `(2,1)`의 각 값이 **오른쪽으로 복제**되어 해당 행에 더해집니다.
1행엔 10이, 2행엔 20이 각각 두 칸에 퍼집니다.
**이게 `y = x @ W + b` 의 `b`(bias)가 batch 방향으로 어떻게 작동하는지** 의 원리입니다.
04장에서 `nn.Linear`이 내부적으로 정확히 이 일을 합니다.
...
- `arange(12)`는 기본 dtype이 `int64`. 신경망에 넣으려면 `float()` 필요. dtype 캐스팅 실수는 매우 흔합니다.
- `.reshape(-1, 784)` 처럼 `-1`을 쓰면 나머지 축을 자동 계산합니다. 06장 CNN에서 이미지를 1차원으로 펼칠 때 씁니다.

## 1.5 원본 예제 관점에서

`simple_classifier.py`로 돌아가 보면, 첫 세 줄이 모두 tensor 생성입니다.
...
`x`가 `(3,1)`, `W`가 `(1,1)`이므로 `x @ W`는 `(3,1)`이 되고, `b`(shape `(1,)`)는
broadcast 되어 각 행에 더해집니다. **shape을 이렇게 추적하는 습관**이 01장의 진짜 목표입니다.
...
Tensor는 "연산 가능한 숫자 격자"이며, 모델이 데이터를 통과하는 동안 **shape이 어떻게 변하는지**를
종이 위에 그릴 수 있으면 딥러닝 코드의 대부분이 예측 가능해집니다.

## 직접 해보기

1. `torch.rand(4, 3)` 으로 4×3 난수 tensor를 만들고, `.mean(dim=0)`으로 **열별 평균**을 구해보세요. 결과는 몇 개의 숫자입니까?
