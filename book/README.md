...
- **지은이 / 대상**: 로봇 비전·물리 AI를 학습 중인 개발자. 파이썬 기초와 `numpy` 행렬 감각이 있다고 가정.
- **검증 환경**: PyTorch **2.8.0+cpu**, Python 3.x, NVIDIA GPU 없음(CPU 전용). `torchvision`·`matplotlib` 미설치.
- **재현 원칙**: 책에 등장하는 **모든 코드 블록과 숫자는 위 환경에서 실제로 실행한 결과**입니다. 임의 수치를 쓰지 않았습니다. 시드 고정 코드(`torch.manual_seed`)는 그대로 재현됩니다.
...
이 폴더에 원래 있던 예제 한 편이 책 전체의 실마리입니다. 4줄이면 읽힙니다.
...
**여기서 이 책이 시작하는 질문 두 가지:**

1. `x=[1,2,3] → y=[5,7,9]`의 진짜 관계는 무엇이고, 왜 답은 `W=2, b=1`(주석)이 아닐까?
   → 실제로는 **`y = 2x + 3`** 입니다. 주석의 "y = W*x + b"는 *모델의 모양*을 쓴 것이지
   정답이 아닙니다. 그런데 학습 후 값은 `b=1.68`로 3에 한참 못 미칩니다. 왜일까?
2. 왜 `W`·`b`를 `no_grad()` 안에서 직접 갱신했을까? PyTorch는 이걸 대신해주지 않을까?

**01장부터 이 두 질문에 순서대로 답하면서, 예제를 "더 가독성 있고 정확한" 형태로 다시 짓습니다.**
04장이 되면 위 코드와 동일한 일을 하는 `nn.Module` 버전을 8줄로 줄일 수 있습니다.

---

## 읽는 방법

- **한 장이 한 개념**입니다. 20~30분이면 한 장. 챕터 끝 "정리"와 "직접 해보기"까지 하는 것을 권장.
- 코드 블록은 **복붙해서 바로 도는** 완전한 조각입니다. 장별로 앞 변수를 재사용하지 않도록 매번 선언합니다.
- GPU가 없어도 전부 CPU에서 돌아갑니다. 시간이 오래 걸리는 예제는 deliberately 작게 설계했습니다.
